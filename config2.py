...
def prediction(model, phi_0,chi_0,P_0):
    ...
    model : MLP
        The trained model that is to be evaluated.
    phi_0 : float
        An input parameter for the model.
    chi_0 : float
        An input parameter for the model.
    P_0 : float
        An input parameter for the model.
    ...
    -------
    list
        A list containing 1024 values, 512 points each for Q and U.
...
def save_run_info(mse, momentum, learning_rate, batch_size, num_epochs, filename=os.path.basename(__file__)):
    ...
    filename : str, optional
        String representing the filename. The default is os.path.basename(__file__).
...
def main():
    ...
    n_inputs = 3
    learning_rate = 0.01
    batch_size = 512 # ideal number from the originator
    num_epochs = 10
    momentum = 0.4 # 0.3, 0.4, 0.5
